...
function loadArticles() {
  ...
  // Check if notion-affiling.js is loaded
  if (typeof loadNotionArticles === 'function') {
    loadNotionArticles(currentFilter, currentSearchQuery, currentPage)
      .then(notionArticles => {
        if (currentPage === 1) {
          articles = notionArticles;
        } else {
          articles = [...articles, ...notionArticles];
        }
        
        renderArticles(articles);
        hasMorePages = notionArticles.length >= 20;
        isLoading = false;
        hideLoadingState();
      })
      .catch(error => {
