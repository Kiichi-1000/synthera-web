#!/usr/bin/env python3
"""ローカルの画像ファイルをdev_projects.jsonに追加"""
import json
import sys
...
FILENAME_TO_PROJECT = {
    "todo.jpg": "ToDoアプリ",
    "calender.jpg": "Calendar App",
    "calendar.jpg": "Calendar App",  # スペル違いにも対応
}

def update_dev_projects_with_local_images(json_path: Path, image_dir: Path):
    """ローカル画像をdev_projects.jsonに追加（相対パスで保存）"""
    ...
    # 対象画像ファイルを確認
    target_files = ["todo.jpg", "calender.jpg", "calendar.jpg"]
    image_files = [image_dir / f for f in target_files if (image_dir / f).exists()]
    ...
    for image_file in image_files:
        ...
        if not project_name:
            ...
            for project in projects:
                ...
                if "todo" in base_name and "todo" in project_name_lower:
                    project_name = project['project_name']
                    break
                elif "calendar" in base_name or "calender" in base_name:
                    if "calendar" in project_name_lower or "calender" in project_name_lower:
                        project_name = project['project_name']
                        break
...
if __name__ == "__main__":
    ...
    json_path = project_root / "data" / "dev_projects.json"
    ...
    updated_count = update_dev_projects_with_local_images(json_path, image_dir)
